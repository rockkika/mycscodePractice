...
auto BufferPoolManager::DeletePage(page_id_t page_id) -> bool {
  ...
  if (const auto frame_found = page_table_.find(page_id); frame_found == page_table_.end()) {
    disk_scheduler_->DeallocatePage(page_id);
    return true;
  }
  if (const auto frame_id = page_table_[page_id];
    frames_[frame_id]->pin_count_.load(std::memory_order_relaxed) == 0) {
...
auto BufferPoolManager::CheckedWritePage(page_id_t page_id, AccessType access_type) -> std::optional<WritePageGuard> {
  ...
  if (const auto page = page_table_.find(page_id); page != page_table_.end()) {
    const auto frame = frames_[page->second];
    frame->pin_count_.fetch_add(1, std::memory_order_relaxed);
    this->replacer_->RecordAccess(page->second,page_id);
    this->replacer_->SetEvictable(page->second,false);
    lock.unlock();
    return WritePageGuard(page_id,frame,replacer_,bpm_latch_,disk_scheduler_);
  }else if (!free_frames_.empty()) {
    const auto free_frame = free_frames_.front();
    free_frames_.pop_front();
    this->replacer_->RecordAccess(free_frame,page_id);
    this->replacer_->SetEvictable(free_frame,false);

    const auto frame = frames_[free_frame];
    frame->Reset();
    auto promise = disk_scheduler_->CreatePromise();
    auto future = promise.get_future();
    std::vector<DiskRequest> requests;
    requests.push_back(
        DiskRequest{false, frame->GetDataMut(), page_id, std::move(promise)});
    disk_scheduler_->Schedule(requests);
    future.get();
    frame->pin_count_.store(1, std::memory_order_relaxed);
    page_table_[page_id] = free_frame;
    lock.unlock();
    return WritePageGuard(page_id,frame,replacer_,bpm_latch_,disk_scheduler_);
  }else {
    const auto victim = this->replacer_->Evict();
    ...
    const auto replace_frame = frames_[frame_id];
    if (replace_frame->is_dirty_) {
      auto promise = disk_scheduler_->CreatePromise();
      auto future = promise.get_future();
      std::vector<DiskRequest> single_task;
      single_task.push_back(DiskRequest{
      true,replace_frame->GetDataMut(),old_page_id,std::move(promise)});
      disk_scheduler_->Schedule(single_task);
      future.get();
    }
    replace_frame->Reset();
    auto promise = disk_scheduler_->CreatePromise();
    auto future = promise.get_future();
    std::vector<DiskRequest> single_task;
    single_task.push_back(DiskRequest{false,replace_frame->GetDataMut(),page_id,std::move(promise)});
    disk_scheduler_->Schedule(single_task);
    future.get();
    page_table_.erase(old_page_id);
    page_table_[page_id] = frame_id;
    replace_frame->pin_count_.store(1, std::memory_order_relaxed);
    this->replacer_->RecordAccess(frame_id,page_id);
    this->replacer_->SetEvictable(frame_id,false);
    lock.unlock();
    return WritePageGuard(page_id,replace_frame,replacer_,bpm_latch_,disk_scheduler_);
  }
}
...
auto BufferPoolManager::CheckedReadPage(page_id_t page_id, AccessType access_type) -> std::optional<ReadPageGuard> {
  std::unique_lock<std::mutex> lock(*bpm_latch_);
  if (const auto frame_found = page_table_.find(page_id); frame_found != page_table_.end()) {
    const auto frame = frames_[frame_found->second];
    frame->pin_count_.fetch_add(1, std::memory_order_relaxed);
    this->replacer_->RecordAccess(frame_found->second,page_id);
    this->replacer_->SetEvictable(frame_found->second,false);
    lock.unlock();
  ...
  if (!free_frames_.empty()) {
    ...
    frames_[free_frame]->Reset();
    auto promise = disk_scheduler_->CreatePromise();
    auto future = promise.get_future();
    std::vector<DiskRequest> single_task;
    single_task.push_back(DiskRequest{false,frames_[free_frame]->GetDataMut(),page_id,std::move(promise)});
    disk_scheduler_->Schedule(single_task);
    future.get();

    frames_[free_frame]->pin_count_.store(1, std::memory_order_relaxed);
    page_table_[page_id] = free_frame;
    lock.unlock();
    return ReadPageGuard(page_id,frames_[free_frame],replacer_,bpm_latch_,disk_scheduler_);
  }
  auto victim = this->replacer_->Evict();
  if (victim == std::nullopt) {
    return std::nullopt;
  }else {
    const auto frame_id = victim.value();
    this->replacer_->RecordAccess(frame_id,page_id);
    this->replacer_->SetEvictable(frame_id,false);
    ...
    auto future = promise.get_future();
    std::vector<DiskRequest> single_task;
    single_task.push_back(DiskRequest{false,frames_[frame_id]->GetDataMut(),page_id,std::move(promise)});
    disk_scheduler_->Schedule(single_task);
    future.get();

    page_table_.erase(old_page_id);
    page_table_[page_id] = frame_id;
    frames_[frame_id]->pin_count_.store(1, std::memory_order_relaxed);
    lock.unlock();
...
auto BufferPoolManager::FlushPageUnsafe(page_id_t page_id) -> bool {
  ...
  const auto frame_found = page_table_.find(page_id);
  if (frame_found == page_table_.end()) {
    return false;
  }
  const auto frame_id = frame_found->second;
  ...
  std::vector<DiskRequest> single_task;
  single_task.push_back(DiskRequest{
  true,frames_[frame_id]->GetDataMut(),page_id,std::move(promise)});
  disk_scheduler_->Schedule(single_task);
  future.get();
  return true;
}
...
auto BufferPoolManager::FlushPage(page_id_t page_id) -> bool {
  ...
  const frame_id_t frame_id = page_it->second;
...
auto BufferPoolManager::GetPinCount(page_id_t page_id) -> std::optional<size_t> {
  ...
  const frame_id_t frame_id = page_it->second;
  return frames_[frame_id]->pin_count_.load(std::memory_order_relaxed);
}
